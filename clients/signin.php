...
session_start();
include 'link.php';
if(isset($_GET['go-to'])&&isset($_SESSION['data'])){
            if (isset($_GET['go-to'])) {
            print_r($_GET);
            echo "<script>window.location.assign('" . $_GET['go-to'] . ".php?data=" . $_GET['data'] . "');</script>";
        } else {
...
if (isset($_POST['login'])) {
    $email = $_POST['email'];
    $pass  = $_POST['password'];
    $sel   = $link->query("SELECT * FROM users WHERE email='$email' AND password='$pass'");
    $row = mysqli_fetch_array($sel);
    $emo = $row['email'];
    $passs = $row['password'];
    if ($email == $emo && $pass == $passs) {
        $selh = $link->query("SELECT * FROM users WHERE email='$emo'");
...
$info = $link->query("SELECT * FROM users");
                            ...
                            <div class="auth-box">
                                <div class="row m-b-20">
                                    <div class="col-md-12">
                                        ...
                                        <a href="../index.php">Guest Login</a>    
